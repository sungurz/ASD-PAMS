...
class AddTenantDialog(tb.Toplevel):
    ...
    def _build_ui(self):
        ...
        # Bottom buttons FIRST so they anchor before notebook expands
        btn_row = tb.Frame(self, padding=(16, 8, 16, 16))
        ...
        self.nb.pack(fill=BOTH, expand=YES, padx=16, pady=(16, 0))
    ...
    def _build_references_tab(self):
        ...
        self._ref_widgets = []
        for i in range(2):
            grp = tb.LabelFrame(f, text=f"Reference {i + 1}", bootstyle="primary")
            grp.pack(fill=X, pady=(0, 10))

            row1 = tb.Frame(grp)
            row1.pack(fill=X, pady=(6, 6), padx=8)
            left = tb.Frame(row1)
            left.pack(side=LEFT, fill=X, expand=YES, padx=(0, 8))
            right = tb.Frame(row1)
            right.pack(side=RIGHT, fill=X, expand=YES)
            ...
            row2 = tb.Frame(grp)
            row2.pack(fill=X, pady=(0, 6), padx=8)
            left2 = tb.Frame(row2)
            left2.pack(side=LEFT, fill=X, expand=YES, padx=(0, 8))
            right2 = tb.Frame(row2)
            right2.pack(side=RIGHT, fill=X, expand=YES)
            ...
            notes_frame = tb.Frame(grp)
            notes_frame.pack(fill=X, padx=8, pady=(0, 8))
            self._lbl_field(notes_frame, "Relation / Notes")
            notes = tb.Entry(notes_frame, font=("Helvetica", 11))
